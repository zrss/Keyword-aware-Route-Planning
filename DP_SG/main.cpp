...
int main() {
	...
	while (true) {
		...
		for (int Node = QueryNode; Node < MAX_NODE_NUM; Node += 20) {
			
			for (int topk = 2; topk <= 10; ++topk) {
				Rating_CoSKQ_Query(Node, Look_For, std::pair<int, int>(MaxDist, MaxPairDist), topk);

				FILE* file = fopen("Result\\TestResult.txt", "a");
				fprintf(file, "\n");
				fclose(file);
			}
		...
		//Rating_CoSKQ_Query(QueryNode, Look_For, std::pair<int, int>(MaxDist, MaxPairDist), topk);
