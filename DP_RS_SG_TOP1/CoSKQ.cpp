...
void findOptimalSolution(int Kpos, int endOfpos, int curMaxPair) {
	...
	while (!RatingPrunning && (EndStartPos[Kpos] !=  NodeNum || (Kpos > 2 && lastPosFlag[Continuous[Kpos]][Kpos] != (flag[Continuous[Kpos]][Kpos][0] + 1)))) {
			...
			for (now = lastPosFlag[cur][Kpos]; !Precorrect && now <= candSize; ++now) {
				...
				if (Precorrect) {
					...
					if (!RatingPrunning) {
						...
						if (CurrentRating + NodeRating[flag[cur][Kpos][now]] > Ratinglower) {

							CurrentRating += NodeRating[flag[cur][Kpos][now]];

							desire->push_back(flag[cur][Kpos][now]);
		...
		if (!RatingPrunning && !Precorrect) {
			
			int xpos = EndStartPos[Kpos];

			for (int end = (*invertList)[Kpos].size(); !NoVisitedCorrect && !RatingPrunning && xpos < end; ++xpos) {
				...
				if (CurrentRating + NodeRating[((*invertList)[Kpos][xpos].mID)] <= Ratinglower) //rating score pruning
					RatingPrunning = true;
			...
			if (NoVisitedCorrect) { //in r-clique
				...
				if (!RatingPrunning) { //no rating pruning
		...
		if (!RatingPrunning && (Precorrect || NoVisitedCorrect)) {
